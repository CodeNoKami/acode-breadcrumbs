...
Symbols › 🍇 usersList.filter › 🔄 arrow
...
- **⚡ Pure Method Chaining Awareness [v2.0.0]:** No generic or messy code blocks in your bar. It safely isolates precise method names inside chains (e.g., extracts just filter, map, or reduce instead of the whole functional block) and intelligently tracks down the original caller identifier (e.g., usersList.filter).
- **🟢 Granular Variable Tracking [v2.0.0]:** Full tracking awareness for standard localized variable definitions (const, let, var), mapping them down carefully within nested blocks.
...
| **Variable [v2.0.0]**     | 🟢      | #7ee787       | Fresh Ecosystem Leaf Green / Block-Scoped Local Variables               |
| **Method Chain [v2.0.0]** | 🍇      | #a371f7       | Elegant Deep Violet / Fluid Pipeline Operators (.map, .filter, .reduce) |
